...
## Acceptance Criteria
- [x] MCP tools are listed with category + enabled status.
- [x] Disabled tools return a deterministic MCP error.
- [x] Settings allow user to opt in/out per tool or category.
- [x] Navigation and basic action tools can drive the GitHub and Agents tabs.
- [x] MCP log includes tool call metadata + timing.
- [x] Tools indicate whether foreground UI is required.
- [x] MCP UI action history is visible in the app.
...
- Expanded Workspaces controls with worktree-name selection and automation mapping.
- Added GitHub favorites/recent PR selection controls + persisted control values.
- Added background-safe state tool alias and surfaced tool safety metadata in Settings.
- Added UI control reference list in Settings.

## Next Steps
✅ Completed

## Open Questions
- Tool permissions remain per-device (UserDefaults) to avoid accidental cross-device automation.
- View/control IDs live in per-feature views + MCP server list (no central registry yet).
- Default behavior now disables mutating tools unless explicitly enabled.
- Headless-safe tools are explicitly marked as background-safe; UI automation remains foreground-only.
- No UI-control-mode indicator yet; revisit if automation usage increases.
