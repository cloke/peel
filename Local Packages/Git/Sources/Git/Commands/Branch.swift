...
extension Commands {
  struct Branch {
    ...
    // git shortlog
    // git shortlog -scen
    static func show(from location: String = "-r", on repository: Model.Repository, callback: (([Model.Branch]) -> ())? = nil) {
      try? Commands.run(.git, command: ["-C", repository.path, "branch", location]) {
        switch $0 {
