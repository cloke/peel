...
- Did you mean \`import { "${importName}" as foo }\`?`,
  ...
  function parseAndChunk(source, language) {
    try {
      ...
      const ast = (0, import_parser.parse)(processedSource, {
        sourceType: "module",
        plugins: [
  ...
  function preprocessGlimmer(source) {
    ...
    for (const range of templateRanges) {
      ...
      processedSource = processedSource.substring(0, adjustedIndex) + placeholder + processedSource.substring(adjustedIndex + range.length);
