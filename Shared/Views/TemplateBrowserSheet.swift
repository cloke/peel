...
//  Peel
//
//  TemplateBrowserDetailView — non-sheet variant for use in a detail pane.
//  (TemplateBrowserSheet was removed as dead code.)
//

import SwiftUI

// MARK: - Template Browser Detail View (inline, non-sheet)

/// Non-sheet variant of the template browser for use in a detail pane.
/// On successful chain launch, navigates to the new chain via the binding.
struct TemplateBrowserDetailView: View {
  @Environment(MCPServerService.self) private var mcpServer
  @Environment(RepositoryAggregator.self) private var aggregator

  var repoPath: String? = nil
  var onChainStarted: ((UUID) -> Void)? = nil
  ...
  private func runSelectedTemplate() {
    ...
    Task {
      ...
      if status == 200 {
        promptText = ""
        selectedTemplate = nil
        // Navigate to the most recently started chain
        if let newestChain = agentManager.chains.first(where: { !$0.state.isTerminal }) {
          onChainStarted?(newestChain.id)
        }
      } else {
